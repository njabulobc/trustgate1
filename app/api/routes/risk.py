...
def _build_risk_response(risk_assessment) -> RiskAssessmentResponse:
    return RiskAssessmentResponse(risk_assessment=RiskAssessmentRead.model_validate(risk_assessment))
...
def create_risk_assessment(
    ...
    try:
        risk_assessment = assess_risk(db=db, client_id=client_id, deal_id=deal_id, actor=current_user.email)
...
def read_latest_risk_assessment(
    ...
    try:
        risk_assessment = get_latest_risk_assessment(db=db, client_id=client_id, deal_id=deal_id)
    except RiskValidationError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc)) from exc
